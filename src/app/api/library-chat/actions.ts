...
export async function renewLoanAction(userName: string, bookTitle: string, specificDueDate?: string) {
  ...
  try {
    ...
    if (foundUsers.length > 1)
      throw new Error(`Múltiplos usuários encontrados com o nome "${userName}". Por favor, seja mais específico.`)
    ...
    if (books.length > 1)
      throw new Error(`Múltiplos livros encontrados com o título "${bookTitle}". Por favor, seja mais específico.`)
    ...
    const newDueDate =
      specificDueDate || new Date(new Date(loanData.due_date).getTime() + 14 * 24 * 60 * 60 * 1000).toISOString()

    // Atualizar data de vencimento
    const { error: updateError } = await supabase.from("loans").update({ due_date: newDueDate }).eq("id", loanData.id)
...
export async function returnLoanAction(loanId: string) {
  ...
  try {
    ...
    const { data: borrowerData } = await supabase.from("users").select("full_name").eq("id", loanData.user_id).single()
