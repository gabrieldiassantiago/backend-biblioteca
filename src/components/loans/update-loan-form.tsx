    ...
    <div className="absolute top-0 right-0 z-50 max-w-md transform transition-all duration-300 ease-in-out animate-fade-in">
      <div className="bg-white border-l-4 border-green-500 rounded-md shadow-lg p-4">
        <div className="flex items-center">
          ...
          <button
            onClick={onClose}
            className="ml-auto p-1.5 text-green-500 hover:bg-green-100 rounded-md"
          >
        ...
        <div className="mt-2 h-1 w-full bg-gray-200 rounded-full overflow-hidden">
          <div className="h-full bg-green-500 rounded-full animate-shrink origin-right" />
        </div>
      </div>
    </div>
  );
...
export function UpdateLoanForm({
  ...
  const handleStatusSubmit = async (
    ...
    try {
      ...
      showSuccessAlert(`Status atualizado para ${newStatus}`);
    } catch (err) {
      console.error(err);
  ...
  const renderOptions = () => {
    switch (currentStatus) {
      ...
      case "returned":
        return (
          <div className="px-3 py-2.5 text-sm italic text-gray-500">
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Textarea
                id="observation"
                placeholder="Comentário sobre devolução (opcional)"
